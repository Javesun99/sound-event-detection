...
def convvit_base_patch16(**kwargs):
    model = ConvViT(
        img_size=[224, 56, 28], patch_size=[4, 2, 2], embed_dim=[64, 256, 512], depth=[1, 1, 1], num_heads=2, mlp_ratio=[4, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

if __name__=='__main__':
    device = 'cuda:0'
    randArray = random.random(size=(1,4,256,4800))
    t1=torch.Tensor(randArray)
    # t1 = t1.to(device)
